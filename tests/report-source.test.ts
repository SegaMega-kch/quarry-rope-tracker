...
test("assembly power audit is transactional, idempotent and preserves prior state on failures", async () => {
  ...
  const assembly = await db.assembly.create({ data: { name: "Audit assembly", lastChangedBy: user.login } });
  const count = () => db.assemblyMovement.count({ where: { assemblyId: assembly.id } });
  await assert.rejects(db.$transaction((tx) => setAssemblyPower(tx, assembly.id, one.id, user)), /горизонт/);
  await assert.rejects(db.$transaction(async (tx) => { await setAssemblyPower(tx, assembly.id, one.id, user, horizon.id); throw new Error("rollback"); }));
  assert.equal(await count(), 0);
  assert.equal((await db.assembly.findUniqueOrThrow({ where: { id: assembly.id } })).horizonId, null);
  await db.$transaction((tx) => setAssemblyPower(tx, assembly.id, one.id, user, horizon.id));
  await db.$transaction((tx) => setAssemblyPower(tx, assembly.id, one.id, user));
  assert.equal(await count(), 1);
  await assert.rejects(db.$transaction((tx) => setAssemblyPower(tx, assembly.id, two.id, user, horizon.id)), /Сначала отключите/);
  ...
  await db.$transaction((tx) => setAssemblyPower(tx, assembly.id, two.id, user, horizon.id));
  await db.$transaction((tx) => setAssemblyPower(tx, assembly.id, null, user));
  await db.$transaction((tx) => setAssemblyPower(tx, assembly.id, null, user));
  assert.equal(await count(), 4);
  const rows = await db.assemblyMovement.findMany({ where: { assemblyId: assembly.id }, orderBy: { id: "asc" } });
  assert.deepEqual(JSON.parse(rows[0].comment!), { kind: "assembly-power-v1", before: null, after: { id: one.id, name: one.name }, horizonBefore: null, horizonAfter: horizon.id });
  assert.equal(rows[2].toPlaceText, two.name);
  assert.equal((await db.assembly.findUniqueOrThrow({ where: { id: assembly.id } })).horizonId, horizon.id);
  await db.assembly.update({ where: { id: assembly.id }, data: { status: "REPAIR" } });
  await assert.rejects(db.$transaction((tx) => setAssemblyPower(tx, assembly.id, one.id, user, horizon.id)), /ремонт/);
  assert.equal(await count(), 4);
});
