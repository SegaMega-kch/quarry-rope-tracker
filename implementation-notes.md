...
## Yakno Horizon Visibility Local Correction 2026-09-19

- After v1.9.0 production use, the user clarified that free Yakno should remain visible next to excavators by physical horizon, even when not connected. Multiple excavators on the same horizon show the same free boxes as reference information, never duplicated inventory. Powered boxes remain attached only to their actual excavator.
- Approved copy: selector label "Запитать ЯКНО", options contain only Yakno numbers; current-horizon list shows free boxes only, with "Нет свободных ЯКНО" when empty. All serviceable free boxes remain selectable from other horizons, moving only on explicit connection.
- Bottom block "ЯКНО без экскаватора" contains only free boxes without a matching active excavator horizon. Sort higher to lower elevation, then number; unknown horizons last. Null horizons are not treated as a shared physical place. Repair stays separate. Summary/share text uses the same placement logic.
- User approved clicking a free box number in an excavator card to select a new horizon without powering it. Reused the existing FREE_HORIZON audit action, added friendly inline errors, serialized writer locking and expected-horizon validation; occupied/repaired/archived boxes cannot be moved by a stale form. Moving or undoing a free box does not change assemblies or excavator horizons.
- Deviation discovered while checking undo: legacy FREE_HORIZON snapshots captured an excavator state although the original operation never changed that state. This action's undo now ignores those unrelated captured states. Stored old history is not rewritten; a regression test verifies a later excavator horizon survives the undo.
- Local-only verification uses a NEW copy in outputs/yakno-horizon-review-20260919 with marked demo excavators 911/912 on +100. Previous review copy and original prisma/review-e2e.db verified unchanged by SHA-256. No production access, Git commit/push, inventory schema migration, MAX call or schedule change in this correction.
- Verification: all 96 tests, focused ESLint and the final production build pass. UI checks pass on headless Edge at 320/390/1400px: two-card visibility, orphan order, number-only dropdown, cross-horizon availability, read-only free list, occupied-box error from another session, free transfer/return, independent assemblies and matching summary. No browser page errors or horizontal overflow. Local preview is http://127.0.0.1:3005/yakno; publication remains pending user review.
- Follow-up visual refinement: horizon is below the excavator name with a 4px gap and the same 16px regular information font as assemblies; orphan-card horizons use that font too. Yakno rows are separated by 4px, with compact 24px-high clickable number targets. Display labels normalize to "Я-117" without rewriting stored identifiers or old history.
- Transfer-button root cause was the orphan-card button selector overriding the primary background. Scoped primary/disabled styles now show an orange active button only after the controlled horizon selection differs from the saved value; reverting disables it again, and clearing a known horizon remains possible. Existing pending/error/success handling is retained.
- Follow-up verification: 97 tests, focused ESLint and production build pass. Browser checks measure 4px row/heading gaps, compare computed horizon typography with assemblies, check the bottom-block button color and enabled/disabled states, repeat the transfer/concurrency scenarios and verify no overflow at 320/390/1400px. Desktop/mobile screenshots inspected. Manual-review horizon moves were preserved; the QA expectation was updated to read current demo positions rather than resetting them. Original and previous review DB hashes remain unchanged. Still local only, with no Git publication, production access or MAX sending.

## Yakno Release v1.9.1 Approval 2026-09-19

- User approved publication and deployment of the reviewed Yakno correction, with one final visual change: 2px between free black numbers, 4px after the powered green number, no underlines. Clicking and keyboard focus remain available.
- Release v1.9.1, "ЯКНО по горизонтам и компактные карточки". No inventory schema migration, database replacement, historical rewrite or report schedule change is required. Production update must use a fresh backup and exact-value comparison, keeping MAX outbox and delivery state intact.
- Final local gate passed: all 97 tests and production build, plus authenticated browser scenarios at 320/390/1400px. Measured row gaps are 4px after green and 2px between black rows; computed underlines are absent. Desktop/mobile screenshots inspected. No page errors or horizontal overflow.
- Deployment results are recorded separately in the ignored operator journal; this approval entry does not claim deployment has completed.
